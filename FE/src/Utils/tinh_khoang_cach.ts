...
export async function Tinh_tong_km(vi_tri_nguoi_dung: any) {
    const toa_do_nguoi_dung = await tinh_toa_do(vi_tri_nguoi_dung);
    if (toa_do_nguoi_dung) {
        ...
        const vi_do_2 = toRad(toa_do_nguoi_dung.lat); // Vĩ độ của điểm 2 chuyển sang radians
        const chenh_lech_vi_do = toRad(toa_do_nguoi_dung.lat - shopLocation[1]); // Sự chênh lệch vĩ độ
        const chenh_lech_kinh_do = toRad(toa_do_nguoi_dung.lng - shopLocation[0]); // Sự chênh lệch kinh độ
