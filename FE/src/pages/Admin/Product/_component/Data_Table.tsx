                ...
                return (
                    <div key={data?._id} className="flex flex-col w-full text-gray-800 border-t border-gray-300 text-sm">
                        <div className="grid grid-cols-[20px_70px_180px_150px_200px_150px_150px_120px] gap-x-4 items-center justify-between p-4">
                            ...
                            ></Checkbox>
                            {/* image */}
                            <img width={100} height={100} className="rounded border" src={data?.image_product} alt="Loading..." />
                            {/* name */}
                            <span className="line-clamp-3">{data?.name_product}</span>
                            {/* category */}
                            <span className="line-clamp-2">{data?.category_id?.name_category}</span>
                            {/* price */}
                            ...
                            <span className="line-clamp-1">{formatDate(data?.createdAt)}</span>
                            {/* options */}
                                    ...
                                    (
                                        data?.attributes?.values?.map((item: any) => (
                                            item?.size?.map((value: any) => (
                                                <div key={item?._id} className="grid border-t duration-200 border-gray-300 gap-x-4 grid-cols-[180px_180px_180px_180px_100px_100px_150px_80px] 
                                                    ...
                                                    <div>
                                                        {
                                                            value?.stock_attribute > 0 ?
                                                                <span className="line-clamp-2">{value?.stock_attribute}</span> :
