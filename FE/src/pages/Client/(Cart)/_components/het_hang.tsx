...
export default function Het_hang({ dataProps }: any) {
    ...
    if (dataProps?.productId?.attributes) {
        const x = dataProps?.productId?.attributes?.values?.find(a => a?.color?.toString() == dataProps?.color_item?.toString());
        const y = x?.size?.find(b => b?.name_size?.toString() === dataProps?.name_size?.toString())
        if (x && y) {
