...
import { Button, Checkbox, Form, FormProps, Input, message, Popconfirm } from "antd"
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm z-50">
            <div className="bg-white p-5 border rounded relative w-[400px] lg:w-[500px]">
                ...
                >
                    ...
                    >
                        <Input className="w-full border px-2 py-2 rounded focus:ring-0" placeholder="Họ và tên" />
                    </Form.Item>
                    ...
                    >
                        <Input className="w-full border px-2 py-2 rounded focus:ring-0" placeholder="Số điện thoại" />
                    </Form.Item>

                    <Form.Item
                        name="addressDetails"
                        className="w-full my-3"
                        rules={[{ required: true, message: 'Please input your address details!' }]}
                    >
                        <Input className="w-full border px-2 py-2 rounded focus:ring-0" placeholder="Tỉnh/Thành phố, Quận/Huyện, Phường/Xã" />
                    </Form.Item>

                    <Form.Item
                        name="addressType"
                        className="w-full my-3"
                        rules={[{ required: true, message: 'Please input your specific address!' }]}
                    >
                        <Input className="w-full border px-2 py-2 rounded focus:ring-0" placeholder="Địa chỉ cụ thể" />
                    </Form.Item>

                    <Form.Item className="w-full my-3">
                        <p>Loại địa chỉ:</p>
                        <div className="flex justify-between gap-4 border rounded px-3 py-2 my-2">
                            <Button className="w-full border-none focus:ring-0">
                                Nhà riêng
                            </Button>
                            <Button className="w-full border-none focus:ring-0">
                                Văn phòng
                    ...
                    <Form.Item className="w-full my-3 flex items-center gap-3">
...
export const List_Address = ({ auth, handleTAdd, handleAddressSelect, handleAddress }: any) => {
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm z-50">
