...
const Header = () => {
        ...
        (typeof window !== 'undefined') && (
            (window.addEventListener('scroll', () => {
                if (toggleFixedHeader.current && toggleForm.current) {
                    (window.scrollY > 100) ?
                        (toggleFixedHeader.current.classList.add('animate-[animationScrollYHeader_1s]', 'lg:-translate-y-3'), toggleForm.current.classList.add('scale-0')) :
                        (toggleFixedHeader.current.classList.remove('animate-[animationScrollYHeader_1s]','lg:-translate-y-3'), toggleForm.current.classList.remove('scale-0'));
                }
            }))
        )
    }, [])
  ...
  return (<>
    <div ref={toggleFixedHeader} className="w-full fixed top-0 bg-white z-[6] shadow-[50px_15px_60px_-15px_rgba(0,0,0,0.3)]">
      <header className="mx-auto relative xl:w-[1440px] flex justify-between items-center mb:w-[95vw] lg:h-20 lg:py-0 py-3">
        {/* menu mobile */}
        <button onClick={toggleMenuMobile} className="*:w-[30px] *:h-[30px] cursor-pointer mb:block lg:hidden">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-menu"><line x1="4" x2="20" y1="12" y2="12" /><line x1="4" x2="20" y1="6" y2="6" /><line x1="4" x2="20" y1="18" y2="18" /></svg>
        </button>

        {/* toggle menu mobile */}
        <div style={{ transform: toggle_Menu_Mobile ? 'translateX(0%)' : 'translateX(-200%)' }} className="lg:hidden fixed w-[40vw] duration-300 z-[-1] pt-[100px] bg-white">
        ...
        <div className="flex items-center gap-x-20">
          {/* logo */}
          <Link to="/" className="lg:relative absolute lg:left-0 lg:translate-x-0 left-[35%] -translate-x-full h-auto mr-2 flex items-start">
            <img
         ...
         *:capitalize *:relative *:duration-300 *:after:content-[''] *:after:duration-300 *:after:absolute *:after:w-0 *:after:right-1/2 *:after:bottom-[-30%] *:after:h-[2px] *:after:bg-orange-600
          *:after:rounded-lg *:before:content-[''] *:before:absolute *:before:h-[2px] *:before:right-0 *:before:bg-orange-600  *:before:bottom-[-30%]  *:before:rounded-lg">
            ...
            <NavLink onClick={ScrollTop_and_Change} className={({ isActive }) => isActive ? "group opacity-100 before:w-full flex items-center group" : "group group flex items-center opacity-[0.70] hover:opacity-100 hover:after:w-full hover:after:right-0"} to={"shops"}>
              shops
            </NavLink>
        ...
        <nav className="flex items-center justify-between *:mx-3 *:duration-300">
          ...
          <div className="cursor-pointer hover:scale-105 duration-300 border">
           <Link to={'/login'} className="bg-black px-4 py-1.5 text-white rounded font-medium text-sm border-none">Login</Link>
          </div>
        </nav>
      </header>
    </div>
    {/* form search mobile */}
    <form ref={toggleForm} className={`relative w-[298px] *:h-[36px] lg:invisible gap-x-2 shadow-2xl mt-6 z-[-1] duration-300 mx-auto top-[50px]`}>
                <input type="text" className="border rounded-full w-full pl-5 pr-14 text-sm outline-none font-normal text-gray-700" placeholder="Search" />
                <button className="absolute top-0 right-[2%] rounded-[50%] w-[36px] duration-300 cursor-pointer">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6 mx-auto">
                        <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
